    ...
    
{
    public partial class Form1 : Form
    {
        ...
        private Button btnAsignarTropas;
        private Button btnAtacar;
        private Button btnMover;
        private Button btnSiguienteTurno;
        private Label lblInfo;
        private Label lblTarjetasInfo;
        private ListBox lstTarjetas;
        private ComboBox cmbTipoTrio;
        private NumericUpDown nudCantidadTropas;
        private ListBox lstLog;
        private ComboBox cmbModoJuego;
        private Button btnIntercambiarTarjetas;
        ...
        public Form1()
        {
            ...
            {
                // Modo Red - Mostrar ventana de conexión
                using (FormConexion formConexion = new FormConexion())
                {
                    ...
                    {
                        MessageBox.Show("Conexión cancelada. Cerrando aplicación.");
                        Application.Exit();
                        return;
                    }
                }
        ...
        private void BtnIniciarJuego_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (modoRed)
                {
                    ...
                    {
                        // Cliente: determinar color según modo
                ...
                switch (mensaje.Tipo)
                {
                    ...
                    case TipoMensaje.IniciarJuego:
                        if (!esServidor && controlador == null)
                        {
                            ...
                            AgregarLog("Juego inicializado por el servidor");
            ...
            else
            {
                ...
                if (nuevoJugador.EsNeutral && nuevoJugador.TropasDisponibles > 0)
                {
                    ...
                    if (exitoNeutral)
                    {
                        ...
                        while (!controlador.DistribucionInicialCompleta())
                        {
                            ...
                            if (!hayTropasPendientes)
                            {
                                AgregarLog("No hay más tropas para colocar");
                                controlador.FinalizarDistribucionInicial();
                                controlador.EstadoActual = EstadoJuego.Refuerzos;
                            ...
                            {
                                AgregarLog($"[ERROR] Error al avanzar turno: {ex.Message}");
                                break;
                            }
                            ...
                            if (siguiente.EsNeutral && siguiente.TropasDisponibles > 0)
                            {
                                AgregarLog($"{siguiente.Nombre} colocando automáticamente...");
                                bool exitoSiguiente = controlador.ColocarTropaInicial(null);
                                if (!exitoSiguiente)
                                {
                                    AgregarLog($"{siguiente.Nombre} no pudo colocar tropa");
                                    break;
                                }
                            ...
                            else
                            {
                                ...
                                AgregarLog($"Siguiente turno para: {siguiente.Nombre}");
                                break;
                            }
                        }
                    }
