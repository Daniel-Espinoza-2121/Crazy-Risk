...
namespace CrazyRisk.Core
{
    public class ControladorJuego
    {
        ...
        public void FinalizarDistribucionInicial()
        {
            ...
            var primerJugador = ObtenerJugadorActual();
            int refuerzos = CalcularRefuerzosParaJugador(primerJugador);
        ...
        public void SiguienteTurnoDistribucion()
        {
            Jugador[] todosJugadores = Jugadores.ObtenerTodos();
            int intentos = 0;
            int maxIntentos = todosJugadores.Length * 2; 
            ...
            if (intentos >= maxIntentos)
            {
                ...
                {
                    if (todosJugadores[i].TerritoriosControlados.Contar > 0)
                    {
                        TurnoActual = i;
                        break;
                    }
        ...
        {
            Jugador[] todosJugadores = Jugadores.ObtenerTodos();
            if (TurnoActual < 0 || TurnoActual >= todosJugadores.Length)
            {
                TurnoActual = 0; // Resetear a primer jugador válido
            }
